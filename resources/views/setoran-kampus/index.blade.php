...
<div class="table-responsive"><table class="table align-middle"><thead><tr><th>No.</th><th>Kode</th><th>Mahasiswa</th><th>Kampus</th><th>Jenis</th><th>Tanggal</th><th>Nominal</th><th>Bukti</th><th></th></tr></thead><tbody>
@forelse ($setoranKampus as $item)<tr><td>{{ $setoranKampus->firstItem() + $loop->index }}</td><td><span class="badge text-bg-light border text-dark">{{ $item->kode_setoran_kampus }}</span></td><td><div>{{ $item->mahasiswa->nama_mahasiswa ?? '-' }}</div><div class="small text-muted">{{ $item->mahasiswa->kode_pmb ?? '-' }}</div></td><td>{{ $item->mahasiswa->kampus->nama_kampus ?? '-' }}</td><td>{{ $item->jenis_setoran }}</td><td>{{ optional($item->tanggal_setor)->format('d M Y') ?? '-' }}</td><td>Rp {{ number_format($item->nominal, 0, ',', '.') }}</td><td class="small">{{ $item->bukti_setor_path ? 'Ada' : '-' }}</td><td class="d-flex gap-1"><a href="{{ route('setoran-kampus.show', $item) }}" class="btn btn-sm btn-outline-primary">Detail</a><a href="{{ route('setoran-kampus.edit', $item) }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-pencil"></i></a></td></tr>@empty<tr><td colspan="9" class="text-center py-5 text-muted">Belum ada data setoran ke kampus.</td></tr>@endforelse
</tbody></table></div><div class="mt-3 d-flex justify-content-between align-items-center flex-wrap gap-2">@include('partials.per-page-select'){{ $setoranKampus->links() }}</div></div></div>
@endsection
