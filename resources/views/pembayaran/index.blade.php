...
@forelse ($pembayarans as $pembayaran)<tr><td>{{ $pembayarans->firstItem() + $loop->index }}</td><td><span class="badge text-bg-light border text-dark">{{ $pembayaran->kode_pembayaran }}</span></td><td><div>{{ $pembayaran->mahasiswa->nama_mahasiswa ?? '-' }}</div><div class="small text-muted">{{ $pembayaran->mahasiswa->kode_pmb ?? '-' }}</div></td><td>{{ $pembayaran->angsuran_ke ?? $pembayaran->jenis_pembayaran }}</td><td>{{ optional($pembayaran->tanggal_bayar)->format('d M Y') ?? '-' }}</td><td>Rp {{ number_format($pembayaran->nominal, 0, ',', '.') }}</td><td><span class="badge text-bg-info">{{ ucfirst(str_replace('_', ' ', $pembayaran->status_bayar)) }}</span></td><td class="small">{{ $pembayaran->bukti_bayar_path ? 'Ada' : '-' }}</td><td><a href="{{ route('pembayaran.show', $pembayaran) }}" class="btn btn-sm btn-outline-primary">Detail</a></td></tr>@empty<tr><td colspan="9" class="text-center py-5 text-muted">Belum ada data pembayaran.</td></tr>@endforelse
