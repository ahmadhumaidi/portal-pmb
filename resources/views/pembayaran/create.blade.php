...
<div class="card dashboard-card border-0 mb-4">
    ...
    <div class="card-body">
        <div class="row g-3">
            ...
            <div class="col-md-6"><label for="jenis_pembayaran" class="form-label">Jenis Pembayaran</label><select id="jenis_pembayaran" name="jenis_pembayaran" class="form-select @error('jenis_pembayaran') is-invalid @enderror" required><option value="">Pilih Jenis Pembayaran</option>@foreach (['Pendaftaran', 'Angsuran', 'Pelunasan', 'Lainnya'] as $jenis)<option value="{{ $jenis }}" @selected(old('jenis_pembayaran', 'Angsuran') === $jenis)>{{ $jenis }}</option>@endforeach</select>@error('jenis_pembayaran')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
            <div class="col-md-6"><label for="angsuran_ke" class="form-label">Angsuran Ke</label><select id="angsuran_ke" name="angsuran_ke" class="form-select @error('angsuran_ke') is-invalid @enderror"><option value="">- Tidak Berlaku -</option>@for ($i = 1; $i <= 12; $i++)<option value="{{ $i }}" @selected((string) old('angsuran_ke') === (string) $i)>Angsuran ke-{{ $i }}</option>@endfor</select>@error('angsuran_ke')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
